...
def get_result(Выручка, Издержки):
...
def input_data():
    ...
    return Выручка, Издержки


def main():
    Выручка, Издержки = input_data()
    get_result(Выручка, Издержки)
...
def get_result_by_person():
    КолВо_Сотрудников = int(input("Введите кол-во сотрудников: "))
    print(
        f"Если разделить прибыль по ровну, то каждый получит: {Результат/КолВо_Сотрудников:.3f} руб.")
...
rezultat_km = int(
    input("Введите результат, который спорцмен должен достигнуть: "))
...
while seychas_km <= rezultat_km:
    ...
    print(f"{day}-ый день: {seychas_km}")
print(
    f"Ответ: на {day}-й день спортсмен достигнет результата - не менее {rezultat_km} км.")
